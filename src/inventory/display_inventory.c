...
void display_item(rpg_t *main, int i, sfVector2f base_position)
{
    ...
    if (item_bank[i].texte != NULL) {
        ...
        sfRenderWindow_drawText(main->window->render_window, item_bank[i].texte,
            NULL);
    }
}
...
{
    sfVector2u win_size = sfRenderWindow_getSize(main->window->render_window);
    ...
    if (main->inventory->state == 1 && strcmp(
        ...
        sfRectangleShape_setPosition(main->inventory->inventory_back,
            middle_position);
        sfRenderWindow_drawRectangleShape(main->window->render_window,
